...
- The help panel gets more of the window, so it no longer cuts off mid-section
  on a laptop screen, and its focus ring is themed rather than Chrome's default
  blue box.
- Store screenshots are generated by `tools/make-screenshots.mjs` from the build
  that actually gets submitted, at twice the pixel density, in a throwaway
  profile with tabs it opens itself. The old ones were stale, showed a help
  screen that no longer existed, and came from lossy captures.
